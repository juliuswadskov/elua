-- lustache: Lua mustache template parsing edited by Tigo.
-- Copyright 2013 Olivine Labs, LLC <projects@olivinelabs.com>
-- Copyright 2020 TigoDevelopment, LLC <me@tigodev.com>
-- MIT Licensed.
...
function CreateScanner(str)
    ...
    -- can be made.
    self.scan_until = function(pattern)
      local match
  ...
  function CreateContext(view, parent)
    ...
    self.push = function(view)
      return CreateContext(view, self)
    end
  
    self.lookup = function(name)
  ...
  function CreateRenderer(...)
    ...
    local error, ipairs, loadstring, pairs, setmetatable, tostring, type = error, ipairs, loadstring, pairs, setmetatable, tostring, type
    local math_floor, math_max, string_find, string_gsub, string_split, string_sub, table_concat, table_insert, table_remove = math.floor, math.max, string.find, string.gsub, string.split, string.sub, table.concat, table.insert, table.remove
    ...
    local function make_context(view)
      ...
      return getmetatable(view) == Context and view or CreateContext(view)
    end
    ...
    self.compile = function(tokens, tags, originalTemplate)
    ...
    self.render = function(template, view, partials)
    ...
    self._section = function(token, context, callback, originalTemplate)
    ...
    self._inverted = function(name, context, callback)
    ...
    self._partial = function(name, context, originalTemplate)
    ...
    self._name = function(name, context, escape)
      local value = context.lookup(name)
    ...
    self.parse = function(template, tags)
      tags = tags or self.tags
      local tag_patterns = escape_tags(tags)
      local scanner = CreateScanner(template)
